...
void fcfsScheduling() {
    ...
    for (i = 0; i < n; i++) {
        cout << "P[" << i << "]\t" << aTime[i] << "\t" << bTime[i] << "\t" << abTime[i] << "\t" << tat_time[i] << "\t" << wTime[i] << "\n";
        // Call the function to print the process timeline
        printProcessTimeline(i, abTime[i] - bTime[i], abTime[i]);
    }
...
void roundRobinScheduling() {
    int limit, total = 0, x, counter = 0, time_quantum;
    ...
    for (total = 0; x != 0;) {
        if (temp_rr[i] <= time_quantum && temp_rr[i] > 0) {
            ...
            cout << "\t\t\t\t" << burst_time_rr[i] << "\t\t\t\t\t" << total - arrival_time_rr[i] << "\t\t\t\t\t" << total - arrival_time_rr[i] - burst_time_rr[i] << "\n";
            printProcessTimeline(i + 1, total - time_quantum, total);
        }
        else if (temp_rr[i] > 0) {
        ...
        if (temp_rr[i] == 0 && counter == 1) {
            x--;
            cout << "\t\t\t\t" << burst_time_rr[i] << "\t\t\t\t\t" << total - arrival_time_rr[i] << "\t\t\t\t\t" << total - arrival_time_rr[i] - burst_time_rr[i] << "\n";
            wait_time += total - arrival_time_rr[i] - burst_time_rr[i];
            turnaround_time += total - arrival_time_rr[i];
            counter = 0;
        }
        ...
        else if (arrival_time_rr[i + 1] <= total) {
            i++;
        }
        else {
            i = 0;
